...
|   √   | get_user_info       | user_id, session_id                                                      | user_name, email, gender, resident_city_id, comment, avatar_url, status                | comment 是个性化签名                                                                                                  |
...
|   ×   | get_friend_msg      | user_id, session_id                                                      | count, [friend_msg_id, friend_user_id, msg_type, msg_content], status                  | 返回用户所有的好友消息（加好友/删好友）, msg_type in ['A', 'D']                                                       |
|   ×   | del_friend_msg      | user_id, session_id, friend_msg_id                                       | status                                                                                 | 删除用户处理过的好友消息，如果同意添加好友则需同时调用 add_friend                                                     |
|   ×   | get_travel_msg      | user_id, session_id                                                      | count, [travel_msg_id, friend_user_id,friend_travel_id, msg_type, msg_content], status | 返回用户所有的行程关联消息（邀请加入行程/加入行程/离开行程/修改行程/删除行程）, msg_type in ['I', 'A', 'L', 'M', 'D'] |
|   ×   | del_travel_msg      | user_id, session_id, travel_msg_id                                       | status                                                                                 | 删除用户处理过的行程关联消息                                                                                          |
...
|   √   | get_travel_group_info_list | user_id, session_id                                                                            | count, [owner_user_id, travel_group_name, travel_group_note, travel_group_color], status | color 是 7 位 char (#FFFFFF)                                              |
...
|   √   | get_travel_info_list       | user_id, session_id, travel_group_id                                                           | count, [travel_id, city_id, date_start, date_end, visibility, travel_note], status       |                                                                           |
...
|   ×   | add_travel_company         | user_id, session_id, travel_id, friend_user_id                                                 | status                                                                                   | 向好友发送邀请请求                                                        |
...
|   √   | address_to_city_list | address             | count, [city_id, country_name, province_name, city_name, latitude, longitude], status | address 是模糊地址字符串，交由 API 处理，返回列表可能为空                 |
...
|   ×   | send_friend_request               | user_id, session_id, friend_user_id, msg_content | status                                                                                              | status = 成功发送请求 or 用户不存在 or 已是好友                                                                                               |
...
|   √   | get_friend_user_info              | user_id, session_id, friend_user_id              | user_id, user_name, gender, resident_city_id, comment, avatar_url, friend_note, status              |                                                                                                                                               |
|   √   | get_others_user_info              | user_id, session_id, other_user_id               | user_id, user_name, (friend_note,) gender, resident_city_id, avatar_url, comment, is_friend, status |                                                                                                                                               |
|   √   | get_others_travel_group_list      | user_id, session_id, other_user_id               | count, [travel_group_id], status                                                                    | 只返回内部有可见 travel 的 travel group                                                                                                       |
|   √   | get_others_travel_group_info_list | user_id, session_id, other_user_id               | count, [travel_group_id, owner_user_id, ...], status                                                | 只返回内部有可见 travel 的 travel group                                                                                                       |
